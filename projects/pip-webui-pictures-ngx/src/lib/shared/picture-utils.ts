...
export function setIconMarginCSS(container, icon) {
  const containerWidth = container.clientWidth ? container.clientWidth : container.width,
...
export function setImageMarginCSS(element, image, params) {
  const containerWidth = element.width ? element.width() : element.clientWidth, // || 80,
  ...
  if (params) {
    cssParams = Object.assign(cssParams, params);
  }
...
export function setErrorIconCSS(element, icon, params) {
  const containerWidth = element.width ? element.width() : element.clientWidth,
